...
func InsertCartItem(item model.RawCartItem) error {
	...
	tx2 := db.MustBegin()
	tx2.MustExec("INSERT INTO shopping_cart VALUES ($1,$2,$3) ON CONFLICT (product) DO UPDATE shopping_cart SET quantity=quantity+$3 WHERE holder=$1 AND product=$2", item.Holder, item.Item, item.Quantity)
